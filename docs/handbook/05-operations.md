...
| `go-sdk/scripts/ix-stage0.sh` | Rebuild **every** `.ext4` tier (`base`, `browser`, `browser-vm`) | Pre-init for all tiers. An old stage0 with a new SDK boots fine but silently skips features it does not know — Volumes attach `/dev/vdc` and get an empty `/data`. |
